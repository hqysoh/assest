...
# ==================== wuyinkeji（速创API）图像接口适配 ====================
# 速创API 与 OpenAI 格式完全不同：
#   1. 异步提交：POST /api/async/image_xxx，JSON body {prompt,size,aspectRatio,urls}，拿到 data.id
#   2. 轮询结果：GET /api/async/detail?key=KEY&id=ID（必须用 query 参数，不能用 json body），
#      data.status: 0初始化 1进行中 2成功 3失败；成功后图片 URL 在 data.result（数组）。
# 参考图：实测 urls 字段【接受 base64 data URI】（data:image/png;base64,...），无需公网图床，
# 因此参考图原样以 data URI 传入，保证「带参考图的四宫格/重绘」一致性，绝不降级为纯文字。

def _is_wuyinkeji(api_url):
    """判断配置的 API 地址是否为速创API（wuyinkeji）。"""
    return 'wuyinkeji.com' in (api_url or '').lower()


# 模型名（用户在设置里填写，大小写/连字符不敏感）→ 速创API 异步端点路径。
# 兼容用户直接填端点名（image_nanoBanana2）或常见别名（nanobanana2 / nano-banana-2 等）。
_WUYIN_IMG_ENDPOINTS = {
    'image_nanobanana2': 'image_nanoBanana2',
    'nanobanana2': 'image_nanoBanana2',
    'image_nanobanana_pro': 'image_nanoBanana_pro',
    'nanobananapro': 'image_nanoBanana_pro',
    'nanobanana_pro': 'image_nanoBanana_pro',
    'image_nanobanana': 'image_nanoBanana',
    'nanobanana': 'image_nanoBanana',
    'image_gpt-image-2': 'image_gpt_image_2',
    'image_gpt_image_2': 'image_gpt_image_2',
    'gpt-image-2': 'image_gpt_image_2',
    'gptimage2': 'image_gpt_image_2',
    'image_wan2.7': 'image_wan2_7',
    'image_wan2_7': 'image_wan2_7',
    'wan2.7': 'image_wan2_7',
    'wan27': 'image_wan2_7',
}


def _wuyinkeji_endpoint(model):
    """把用户填的模型名解析为速创API 图像异步端点（默认 NanoBanana2）。"""
    key = re.sub(r'[\s]', '', (model or '').strip().lower())
    if key in _WUYIN_IMG_ENDPOINTS:
        return _WUYIN_IMG_ENDPOINTS[key]
    # 用户已直接填了 image_xxx 端点名（保留原大小写）
    if (model or '').strip().lower().startswith('image_'):
        return model.strip()
    return 'image_nanoBanana2'


def _wuyinkeji_size_ratio(size):
    """把 OpenAI 风格的 size（1024x1024 / auto）映射为速创API 的 size(1K/2K/4K) + aspectRatio。"""
    s = (size or 'auto').strip().lower()
    if s in ('auto', ''):
        return '2K', 'auto'
    # 形如 WxH：用宽高近似比例 + 像素估算清晰度
    m = re.match(r'^(\d+)\s*[x×]\s*(\d+)$', s)
    if m:
        w, h = int(m.group(1)), int(m.group(2))
        longest = max(w, h)
        res = '1K' if longest <= 1024 else ('2K' if longest <= 2048 else '4K')
        from math import gcd
        g = gcd(w, h) or 1
        ratio = f'{w // g}:{h // g}'
        # 仅保留接口支持的常见比例，否则回退 auto
        if ratio not in ('1:1', '16:9', '9:16', '4:3', '3:4', '3:2', '2:3', '5:4', '4:5', '21:9'):
            ratio = 'auto'
        return res, ratio
    if s in ('1k', '2k', '4k'):
        return s.upper(), 'auto'
...
def _call_wuyinkeji_image(prompt, image_b64_list, api_url, api_key, model, size, n=1):
    """调用速创API 图像接口（异步提交 + 轮询结果）。返回 (images_b64, error)。
    image_b64_list: 参考图 base64（不含 data: 前缀），转为 data URI 放进 urls。"""
    base = (api_url or '').rstrip('/')
    # 用户可能填到 /v1 或带尾巴，统一取到域名根
    m = re.match(r'^(https?://[^/]+)', base)
    root = m.group(1) if m else base
    endpoint = _wuyinkeji_endpoint(model)
    submit_url = f'{root}/api/async/{endpoint}'
    detail_url = f'{root}/api/async/detail'
    headers = {'Authorization': api_key, 'Content-Type': 'application/json'}

    res_size, ratio = _wuyinkeji_size_ratio(size)
    urls = []
    for b64 in (image_b64_list or [])[:7]:   # 接口参考图上限（NanoBanana 系列 7~14，保守取 7）
        if b64:
            urls.append(f'data:image/png;base64,{b64}')
    body = {'prompt': prompt, 'size': res_size, 'aspectRatio': ratio}
    if urls:
        body['urls'] = urls

    print(f"[WUYIN] submit {submit_url} model={endpoint} size={res_size} ratio={ratio} refs={len(urls)}")
    try:
        r = requests.post(submit_url, json=body, headers=headers, timeout=60)
    except Exception as e:
        return None, f'提交失败: {e}'
    if r.status_code != 200:
        return None, f'提交失败 HTTP {r.status_code}: {(r.text or "")[:200]}'
    try:
        sub = r.json()
    except Exception:
        return None, f'提交返回非 JSON: {(r.text or "")[:200]}'
    if sub.get('code') != 200:
        return None, f"提交被拒: {sub.get('msg') or sub}"
    task_id = (sub.get('data') or {}).get('id') if isinstance(sub.get('data'), dict) else None
    if not task_id:
        return None, f'未拿到任务ID: {sub}'

    # 轮询结果（图像一般 10~60s；最多等 5 分钟）
    deadline = time.time() + 300
    last_msg = ''
    while time.time() < deadline:
        time.sleep(3)
        try:
            # 结果查询必须用 GET + query 参数（key & id）；用 json body 会报 id 缺失
            dr = requests.get(detail_url, params={'key': api_key, 'id': task_id}, headers=headers, timeout=30)
        except Exception as e:
            last_msg = f'查询异常: {e}'
            continue
        if dr.status_code != 200:
            last_msg = f'查询 HTTP {dr.status_code}'
            continue
        try:
            dj = dr.json()
        except Exception:
        ...
        if status == 2:   # 成功
            imgs = _wuyinkeji_extract_images(data)
            if imgs:
                return imgs, None
            return None, f'成功但未解析到图片地址: {json.dumps(data, ensure_ascii=False)[:300]}'
        if status == 3:   # 失败
            return None, f"生成失败: {data.get('message') or dj.get('msg') or '未知'}"
        last_msg = f'status={status}'
    return None, f'生成超时（{last_msg}）'


def _wuyinkeji_extract_images(data):
    """从结果查询的 data 中尽量提取图片 URL/base64，下载/解码为 base64 列表。
    文档未固定字段名，故宽松扫描常见字段与任意 http(s) 图片链接。"""
    candidates = []
    ...
    # 优先常见结果字段
    for key in ('result', 'results', 'urls', 'url', 'images', 'image', 'output', 'data'):
        if key in data:
            _collect(data[key])
    if not candidates:
        _collect(data)
...
def _call_image_edit_api(prompt, image_b64_list, api_url, api_key, model='gpt-image-2', size='auto', quality='auto', n=1):
    ...
    返回 (images_b64, error)。
    若配置的是速创API（wuyinkeji），自动切换到其异步图像接口。"""
    # —— 速创API 适配分流 ——
    if _is_wuyinkeji(api_url):
        return _call_wuyinkeji_image(prompt, image_b64_list, api_url, api_key, model, size, n)
...
def _call_image_api(prompt, api_url, api_key, model, size, quality, n=1):
    ...
    若配置的是速创API（wuyinkeji），自动切换到其异步图像接口（纯文生图）。"""
    if _is_wuyinkeji(api_url):
        return _call_wuyinkeji_image(prompt, [], api_url, api_key, model, size, n)
