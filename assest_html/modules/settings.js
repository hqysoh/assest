...
<option value="yusu" ${(s.videoDefaults||{}).workflow==='yusu'?'selected':''}>Yusu 导演台</option>
    ...
    LLM_DEFAULTS: {
        ...
        expandPrompt: '你是专业的影视分镜师。下面给你整部剧本作为背景参考。请把用户提供的这一条【当前分镜】，拆解成【4个连续镜头】（构成 2×2 四宫格，顺序为左上→右上→左下→右下），作为「当前分镜」过渡到「下一个分镜」之间的衔接画面：第1格紧接当前分镜的画面、4格之间动作/镜头平滑过渡、最后第4格自然过渡到下一个分镜的画面，让当前分镜与下一个分镜之间衔接顺畅。全程保持人物外形服装画风光线一致。每格只描述该格画面，避免字幕文字。\n严格要求：只输出 4 行，每行一格的画面提示语，不要编号、不要解释、不要空行。\n\n【剧本背景参考】\n{script}',
    },
